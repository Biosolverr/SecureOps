...
describe("SecureVault", function () {
  ...
  describe("Initial State", function () {
    ...
    it("3b. Should revert with zero address owner", async function () {
      const SecureVault = await ethers.getContractFactory("SecureVault");
      await expect(
        upgrades.deployProxy(SecureVault,
    ...
    it("3c. Should revert with duplicate roles", async function () {
      ...
      await expect(
        upgrades.deployProxy(SecureVault,
          ...
          { kind: "uups" })
      ).to.be.revertedWithCustomError(vault, "DuplicateRoles");
    });
  ...
  describe("Deposits and Locking", function () {
    it("4. Should allow ETH deposit and transition to FUNDED", async function () {
      ...
      await expect(vault.deposit({ value: depositAmount })).to.emit(vault, "Deposited");
    ...
    it("6. Should prevent non-owner from locking", async function () {
      await vault.deposit({ value: ethers.parseEther("1.0") });
      await expect(vault.connect(otherAccount).lock())
    ...
    it("6c. Should prevent deposit during quarantine", async function () {
      ...
      await expect(vault.deposit({ value: ethers.parseEther("1.0") }))
        .to.be.revertedWithCustomError(vault, "QuarantineActive");
    });

    it("6d. inState modifier reverts with InvalidState (not InvalidAddress)", async function () {
      // Vault is in INIT — calling lock() which needs FUNDED should emit InvalidState
      await expect(vault.lock()).to.be.revertedWithCustomError(vault, "InvalidState");
    });
  });
  ...
  describe("Execution Logic", function () {
    ...
    it("7. Should transition to EXECUTION_PENDING with correct secret", async function () {
      ...
      await expect(vault.initiateExecution(secret)).to.emit(vault, "SecretRevealed");
    ...
    it("8. Should revert execution before lock duration expires", async function () {
      await expect(vault.initiateExecution(secret))
        .to.be.revertedWithCustomError(vault, "LockPeriodNotOver");
    });

    it("9. Should revert execution with wrong secret", async function () {
      await time.increase(lockDuration);
      const wrongSecret = ethers.encodeBytes32String("wrong");
      await expect(vault.initiateExecution(wrongSecret))
        .to.be.revertedWithCustomError(vault, "InvalidSecret");
    });
    ...
    it("10b. Should only transfer depositedEthAmount, not extra ETH in contract", async function () {
      await time.increase(lockDuration);
      await vault.initiateExecution(secret);

      // Send extra ETH directly to vault (e.g. from quarantine stake)
      await otherAccount.sendTransaction({ to: await vault.getAddress(), value: ethers.parseEther("0.5") });
      ...
      expect(counterpartyAfter).to.be.closeTo(
        counterpartyBefore + ethers.parseEther("1.0"),
  ...
  describe("Refund and Delay", function () {
    ...
    it("12. Should revert unauthorized refund", async function () {
      await vault.deposit({ value: ethers.parseEther("1.0") });
      await expect(vault.connect(otherAccount).refund())
  ...
  describe("Quarantine and Attack Simulations", function () {
    it("13. Should allow quarantine with exact stake", async function () {
    ...
    it("15. Direct ETH send should NOT trigger quarantine (receive() is passive)", async function () {
      const endTimeBefore = await vault.quarantineEndTime();
      await otherAccount.sendTransaction({ to: await vault.getAddress(), value: QUARANTINE_STAKE });
      // quarantine state must NOT change
      expect(await vault.quarantineEndTime()).to.equal(endTimeBefore);
      expect(await vault.quarantineInitiator()).to.equal(ethers.ZeroAddress);
    });

    it("16. Invariant: deposited amount zero after execute", async function () {
      await vault.deposit({ value: ethers.parseEther("1.0") });
      await vault.lock();
      await time.increase(lockDuration);
      await vault.initiateExecution(secret);
    ...
    it("17. Owner can release quarantine early and stake is refunded to initiator", async function () {
      const balanceBefore = await ethers.provider.getBalance(otherAccount.address);

      await vault.connect(otherAccount).initiateQuarantine({ value: QUARANTINE_STAKE });
      expect(await vault.quarantineEndTime()).to.be.gt(await time.latest());

      await expect(vault.releaseQuarantine()).to.emit(vault, "QuarantineReleased");

      const endTime = await vault.quarantineEndTime();
      const latest = await time.latest();
      expect(endTime).to.be.lte(latest);

      // Stake refunded to initiator
    ...
    it("18. Non-owner cannot release quarantine", async function () {
      await vault.initiateQuarantine({ value: QUARANTINE_STAKE });
      await expect(vault.connect(otherAccount).releaseQuarantine())
        .to.be.revertedWithCustomError(vault, "OwnableUnauthorizedAccount");
    });

    it("18b. Cannot initiate quarantine twice while active", async function () {
      await vault.initiateQuarantine({ value: QUARANTINE_STAKE });
      await expect(vault.connect(otherAccount).initiateQuarantine({ value: QUARANTINE_STAKE }))
        .to.be.revertedWithCustomError(vault, "QuarantineActive");
    });
  ...
  describe("Pausable", function () {
    ...
    it("34. Non-owner cannot pause", async function () {
      await expect(vault.connect(otherAccount).pause())
        .to.be.revertedWithCustomError(vault, "OwnableUnauthorizedAccount");
    });
  ...
  describe("Recovery Account", function () {
    ...
    async function buildDigest(vaultAddr, newOwner, nonce, deadline, chainId) {
      const RECOVERY_TYPEHASH = ethers.keccak256(
        ethers.toUtf8Bytes("Recovery(address newOwner,uint256 nonce,uint256 deadline)")
      );
      const structHash = ethers.keccak256(
        ethers.AbiCoder.defaultAbiCoder().encode(
          ["bytes32", "address", "uint256", "uint256"],
          [RECOVERY_TYPEHASH, newOwner, nonce, deadline]
        )
      );
      ...
      const domainSeparator = ethers.keccak256(
        ethers.AbiCoder.defaultAbiCoder().encode(
          ...
          [
            DOMAIN_TYPEHASH,
            ethers.keccak256(ethers.toUtf8Bytes("SecureVault")),
    ...
    it("21. Should recover account with valid EIP-712 signatures", async function () {
      ...
      const digest = await buildDigest(await vault.getAddress(), newOwner, currentNonce, deadline, chainId);
    ...
    it("22. Should revert with expired deadline", async function () {
      ...
      const digest = await buildDigest(await vault.getAddress(), otherAccount.address, currentNonce, deadline, chainId);
      const ownerSig = ethers.Signature.from(ownerKey.sign(digest)).serialized;
      const guardianSig = ethers.Signature.from(guardianKey.sign(digest)).serialized;
    ...
    it("24b. Should revert if both signatures are identical (same key used twice)", async function () {
      const newOwner = otherAccount.address;
      const deadline = (await time.latest()) + 3600;
      const currentNonce = await vault.nonce();
      const chainId = (await ethers.provider.getNetwork()).chainId;
      ...
      // Pass same sig as both owner and guardian — must revert
      await expect(vault.recoverAccount(newOwner, deadline, ownerSig, ownerSig))
  ...
  describe("Upgrade Security", function () {
    it("25. Should schedule upgrade with timelock", async function () {
      ...
      await expect(vault.scheduleUpgrade(newImpl)).to.emit(vault, "UpgradeScheduled");
    });
